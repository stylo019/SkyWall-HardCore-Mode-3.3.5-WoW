...
local function PlayerOnDeath(event, killer, player)
    local inBattleGround = player:InBattleground()
    if not inBattleGround then
        if player:HasItem(666, 1) then
            ...
            if guild and guild:GetName() == "HardCoded" then
                ...
                SendWorldMessage("|cFFffffffHardcore|r : |cFF007bf6" ..
                    playerName .. " was removed from the Hardcore Guild!|r")
            end
            ...
            for _, p in ipairs(players) do
                p:SendBroadcastMessage("|cFFffffffHardcore|r : |cFFffffffUser |cFF00ff00" ..
                    playerName ..
                    "|r |cFFffffffwas killed by |cFF00ff00" ..
                    killerName .. "|r  - |cFFffffffLevel " .. playerLevel .. "")
                p:SendAreaTriggerMessage("|cFFffffffHardcore|r : |cFFffffffUser |cFF00ff00" ..
                    playerName ..
                    "|r |cFFffffffwas killed by |cFF00ff00" ..
                    killerName .. "|r  - |cFFffffffLevel " .. playerLevel .. "")
            end
            local input_HC_Dead = "INSERT INTO hc_dead_log (username, level, killer, date, result, guid) VALUES ('" ..
                player:GetName() ..
                "', '" .. player:GetLevel() .. "', '" .. killer:GetName() .. "',  NOW(), 'DEAD', '" .. playerGUID .. "')"
            AuthDBExecute(input_HC_Dead)

            local embed = '{"username": "Hardcore System", "avatar_url": "https://i.imgur.com/5HsZDQq.png", "content": "**HardCore:** Player **'.. playerName ..'** was killed by **' ..killerName.. '** at Level **'.. playerLevel ..'** ...better luck next time!"}'
            -- POST example with JSON request body
            HttpRequest("POST", "https://discord.com/api/webhooks/1156831672424222720/V9psXrzc5aftM3sqROAnWXU7fjQ0wAws7nGt8DsbJ58VYoMA2g3pih5E8HZ10GlK7Woc",
...
local function OnFirstTalk(event, player, unit)
    if player:GetLevel() == 1 then
        ...
        player:SendBroadcastMessage(
            "|cFFffffffHardcore|r : Your current level is too high to participate in HC mode. In order to experience the thrill of HC, it is necessary to create a new hero.")
...
local function OnHardCore(event, player, unit, sender, intid, code)
    if intid == 2 then
        ...
        player:SetCoinage(0)
        player:SendAreaTriggerMessage("|cFFffffffWelcome to Hardcore Mode,|cFF00ff00" ..
            player:GetName() .. ".|r |cFFffffffStay vigilant and tread carefully!|r")
        SendWorldMessage("|cFFffffffHardcore|r : |cFF00ff00" ..
            player:GetName() .. "|r has entered Hardcore Mode! Best of luck on your journey!")

        local playerGUID = player:GetGUIDLow()

        -- Insert a record into the hc_dead_log table to mark the player's start
        local input_HC_Start = "INSERT INTO hc_dead_log (username, level, killer, date, result, guid) VALUES ('" ..
            player:GetName() .. "', '" .. player:GetLevel() .. "', 'STARTED', NOW(), 'BEGIN', '" .. playerGUID .. "')"
        AuthDBExecute(input_HC_Start)

        -- Add the player to the "HardCore" guild using Guild:AddMember()
        local guild = GetGuildByName("HardCoded")
        if guild then
...
RegisterPlayerEvent(6, PlayerOnDeath)
RegisterPlayerEvent(8, PlayerOnDeath)
